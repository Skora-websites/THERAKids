...
const fallbackPosts = {
  ...
  'speech-milestones-toddlers': {
    ...
    content: `
      ...
      <h2>By 2 years</h2>
      <p>Word combinations appear: "more milk," "daddy go." Children this age typically understand around 200 to 300 words, follow two-step instructions, and ask simple questions like "what's that?"</p>
      <h2>By 3 years</h2>
      ...
      <p>If your toddler uses few gestures by 12 months, has no words by 16 months, or loses skills at any age, an early speech-language assessment is worthwhile. Early support works best, and an evaluation simply gives you information, so there is no downside to asking.</p>
    `
  }
};
...
const BlogPost = () => {
  ...
  const effectiveBlog = status === 'loading' ? null : (blog || fallbackPosts[slug]);

  useEffect(() => {
    if (!effectiveBlog) return undefined;
    const metaTitle = (effectiveBlog.meta_title || '').trim();
    const keywords = (effectiveBlog.meta_keywords || '').trim();
    const metaDescription = (effectiveBlog.meta_description || '').trim();
    setSeo({
      title: metaTitle || effectiveBlog.title || '',
      keywords, // empty → tag omitted
      description: metaDescription || makeExcerpt(effectiveBlog.content, 160),
      // Per-article Open Graph / canonical overrides
      type: 'article',
      image: effectiveBlog.featured_image || '',
      canonical: (effectiveBlog.canonical_url || '').trim()
    });
    return () => resetSeo();
  }, [effectiveBlog]);
  ...
  // (effectiveBlog is computed above so the SEO effect can cover both paths.)
